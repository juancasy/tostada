# Clase MesaOSCModelLoader

MesaOSCModelLoader <- R6Class("MesaOSCModelLoader",
	private = list(
	...
	public = list(
		...
		Load = function(strFile){
			...
			if(noAdiabaticas){
				...
				indices <- c(rmasa=1, rfot=2, rlumi=3, al=6, edad=11, rot=12, r=1, q=2, t=3, p=4, rho=5, gamma=10, aast=15)
			}else{
				contenido <- "Adiabatic"
				# define los índices de los datos necesarios en el caso adiabático
				indices <- c(rmasa=1, rfot=2, rlumi=3, al=6, edad=11, rot=13, r=1, q=2, t=3, p=4, rho=5, gamma=10, aast=15)
			}			
...
MesaOSCModelLoader.Test <- function(path, isUrl){
	r <- FALSE	
	c <- NULL			
